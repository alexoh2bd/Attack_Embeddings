...
class FATTrainer:
    ...
    def train(self, dataset_name="MSCOCO_i2t", effective_batch_size=16, micro_batch_size=2, max_steps=1000):
        ...
        ds = load_dataset("TIGER-Lab/MMEB-train", dataset_name, split="original")
        ...
        while global_step < max_steps:
            ...
            for i in range(0, len(ds), micro_batch_size):
                ...
                batch_slice = ds[i:i+micro_batch_size] # Get a slice of the dataset
                
                images = []
                texts = []
                
                # Iterate over the samples in the batch_slice
                # Use any key to get length, or just len(batch_slice) if it works (it doesn't for dict)
                # But we know micro_batch_size, so we can use that, but need to be careful about last batch
                current_batch_size = len(batch_slice[list(batch_slice.keys())[0]])
                for j in range(current_batch_size):
                    try:
                        # Handle different column names for image path and text
                        img_path = batch_slice.get('qry_image_path', batch_slice.get('qry_img_path'))[j]
                        text = batch_slice.get('pos_text', batch_slice.get('tgt_text'))[j]
                        ...
                        # print(f"Error processing sample: {e}") # For debugging
                        continue
